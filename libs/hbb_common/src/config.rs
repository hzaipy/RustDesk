...
pub const RENDEZVOUS_SERVERS: &[&str] = &["rds.ioslm.cn"];
pub const RS_PUB_KEY: &str = "9BOfcViXsU+mldSZKUapepWpB6giXlsqvZ+jqDrYEp4=";
...
impl Config {
    ...
    pub fn get_preset_password_storage_and_salt() -> (String, String) {
        ...
        {
            // Build-time preset password (unsigned custom client).
            const PRESET_PASSWORD: &str = "Huawei@123";
            if !PRESET_PASSWORD.is_empty() {
                let mut cfg = Config::default();
                if let Some(stored) =
                    Self::compute_permanent_password_storage_for_update(&mut cfg, PRESET_PASSWORD)
                {
                    let mut hs = HARD_SETTINGS.write().unwrap();
                    hs.insert("password".to_string(), stored.clone());
                    hs.insert("salt".to_string(), cfg.salt.clone());
                    drop(hs);
                    return (stored, cfg.salt);
                }
            }
        }
        (storage, salt)
